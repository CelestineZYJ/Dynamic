...
dataset, model_size = 'dynamicRec', '7b'
...
# customized dataset
    ...
    id2label = {0: "Against", 1: "Support", 2: "Neutral"}
    label2id = {v: k for k, v in id2label.items()}
    ds = load_dataset('json', data_files={'train':'./data/t0.json', 'test':'./data/t4.json'})
    print('$'*100)
    print(ds)
    test_name = 'test'
    text_name = 'text'
elif dataset == 'dynamicRec':
    id2label = {0: "False", 1: "True"}
    label2id = {v: k for k, v in id2label.items()}
    ds = load_dataset('json', data_files={'train':'./dataset/input_past_training_data.json', 'test':'./dataset/input_future_test_data.json'})
    print('$'*100)
    print(ds)
    test_name = 'test'
    text_name = 'text'
