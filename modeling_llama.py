...
class LlamaForRecommendation(LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = LlamaModel(config)
    ...
    def forward(
        ...
        predict_interact_label = self.sigmoid(pooled_logits)
        
        # print('is in train: '+str(self.is_in_train))
        
        recordf = open('./dataset/predict_true.csv', 'a')

        # print(str(round(100*float(predict_interact_label), 5))+' groundtruth: '+str(int(labels)))
        recordf.write(str(int(labels))+' '+str(round(1000*float(predict_interact_label), 5))+'\n')
        recordf.close()
        ...
        if labels is not None:
            ...
            #     loss_fct = CrossEntropyLoss()
                ...
                loss = weights[1] * labels * torch.log(torch.clamp(predict_interact_label, min=1e-15, max=1)) + \
                weights[0] * (1 - labels) * torch.log(torch.clamp(1 - predict_interact_label, min=1e-15, max=1))
                loss = -loss[0][0]
                # print(loss)
            # elif self.config.problem_type == "multi_label_classification":
            #     loss_fct = BCEWithLogitsLoss()
